...
class FP16ComplexAdderGoldenModel:
    ...
    def complex_add(self, a_real_value, a_imag_value, b_real_value, b_imag_value):

        # Complex Add (a+bi) + (c+di) = (a+c)+(c+d)i
    
        aout_r = self.add(a_real_value, b_real_value)
        aout_i = self.add(a_imag_value, b_imag_value)

        return aout_r, aout_i     

# Load inputs and calculate results
input_opa_file = "../data/cmac_opc.txt"
input_opb_file = "../data/cmac_opd.txt"
output_file = "../data/py_cmac_add_output.txt"

complex_add = FP16ComplexAdderGoldenModel(rounding_mode="nearest", ieee_mode="simplified")
...
with open(input_opa_file, "r") as opa_file, open(input_opb_file, "r") as opb_file, open(output_file, "w") as outfile:
    for a_bin, b_bin in zip(opa_file, opb_file):
        ...
        a_real_value = complex_add.binary_to_fp16(a_real)
        a_imag_value = complex_add.binary_to_fp16(a_imag)
        b_real_value = complex_add.binary_to_fp16(b_real)
        b_imag_value = complex_add.binary_to_fp16(b_imag)

        aout_r_dec, aout_i_dec = complex_add.complex_add(a_real_value, a_imag_value, b_real_value, b_imag_value)

        aout_r_bin = complex_add.fp16_to_binary(aout_r_dec)
        aout_i_bin = complex_add.fp16_to_binary(aout_i_dec)

        outfile.write(aout_r_bin + aout_i_bin + "\n")

print(f"\033[32mComplex add results saved to {output_file}.\033[0m")

# a_bin = "0000111101111100"
# b_bin = "1100010000000000"

# a_value = complex_add.binary_to_fp16(a_bin)
# b_value = complex_add.binary_to_fp16(b_bin)

# print(a_value)
# print(b_value) 

# ares_value = complex_add.add(a_value, b_value)
# print(ares_value)
# res_bin = complex_add.fp16_to_binary(ares_value)
# print(res_bin)

# a_bin = "1100000000000000"
# b_bin = "1100010000000000"

...
class FP16DividerGoldenModel:
    ...
    def divide(self, a_value, b_value):

        # Perform the actual divide
        if b_value == 0.0:
            if a_value == 0.0:
                result = float('inf')
            else:
                result = float('inf') if math.copysign(1.0, a_value) == math.copysign(1.0, b_value) else float('-inf')
        elif math.isinf(a_value) and math.isinf(b_value):
            result = float('inf') if math.copysign(1.0, a_value) == math.copysign(1.0, b_value) else float('-inf')
        else:
...
output_file = "../data/py_fp16div_output.txt"

divider = FP16DividerGoldenModel(rounding_mode="nearest", ieee_mode="simplified")

with open(input_file, "r") as infile, open(output_file, "w") as outfile:
    for line in infile:
        a_bin, b_bin = line.strip().split()
        a_value = divider.binary_to_fp16(a_bin)
        b_value = divider.binary_to_fp16(b_bin)
        result_dec = divider.divide(a_value, b_value)
        result_bin = divider.fp16_to_binary(result_dec)
        outfile.write(result_bin + "\n")
    print("\033[32mAll results have been generated\033[0m")

# a_bin = '1111111101111001'
# b_bin = '1111111000111010'
# a_value = divider.binary_to_fp16(a_bin)
# b_value = divider.binary_to_fp16(b_bin)
...
# result_dec = divider.divide(a_value, b_value)
# print(result_dec)
# print(a_value/b_value, math.copysign(1.0, a_value/b_value))
# result_bin = divider.fp16_to_binary(result_dec)

# print(math.copysign(1.0, float('inf')))
# print(math.copysign(1.0, float('-inf')))
# print(math.copysign(1.0, float('nan')))
# print(math.copysign(1.0, 0.0))
# print(math.copysign(1.0, -0.0))
# print(math.copysign(1.0, -0.0*float('-inf')))

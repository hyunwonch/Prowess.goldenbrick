classdef chirpParams_est < baseProcessors.baseTrigger
    ...
    methods
        ...
        function out = process(self)
            ...
            if available_samples >= self.config.window_size
                ...
                chirp_mon = z.*(est_chirp);
                chirp_mon = (sum(abs(chirp_mon),2));
                chirp_mon = self.exp_avg_monitor.work(chirp_mon);
            ...
            % out.est_params goes to downstream nodes
            out.output_params.fc = fc_est; 
            out.output_params.chirp_rate = cr_est; 

            % update log file
            self.update_log(val, fc_est, cr_est);

            %% estimator confidence metric
            self.est_vals = [cr_est, fc_est];

            %confidence_val = self.confidence_computation(self.cr_est, self.fc_est);
        ...
        % function to compute confidence metrics

        function conf = confidence_computation(self, cr_est, fc_est)

            calc_window = 10; % Look at the last 10 calculations - TESTING (will be in cfg)

            % Check if we have developed 10 estimates yet, if yes,
            % calculate our confidence metric
            if  nnz(~isnan(cr_est)) && nnz(~isnan(fc_est)) > calc_window
                cr_est = var(self.cr_est(~isnan(self.cr_est)));
                fc_est = var(self.fc_est(~isnan(self.fc_est)));

                self.cr_var(end+1) = cr_est;
                self.fc_var(end+1) = fc_est;
            end

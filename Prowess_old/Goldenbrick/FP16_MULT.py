...
class FP16MultiplierGoldenModel:
...
output_file = "../data/py_fp16mult_output.txt"

multiplier = FP16MultiplierGoldenModel(rounding_mode="nearest", ieee_mode="simplified")

with open(input_file, "r") as infile, open(output_file, "w") as outfile:
    for line in infile:
        a_bin, b_bin = line.strip().split()
        a_value = multiplier.binary_to_fp16(a_bin)
        b_value = multiplier.binary_to_fp16(b_bin)
        result_dec = multiplier.multiply(a_value, b_value)
        result_bin = multiplier.fp16_to_binary(result_dec)
